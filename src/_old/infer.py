...
if __name__ == '__main__':
    ...
    dev2_data = load_data(meetings_dir="/mnt/scratch/tmp/ipoloka/meeting_data/dev_set/240415.2_dev_with_GT/MTG",
    ...
    for index, session in tqdm.tqdm(sessions.iterrows(), total=len(sessions)):
        ...
        if not os.path.exists(out_tcp_file):
            ...
            if infer_args.use_gt_diar:
                ...
                diar_json_path = os.path.dirname(
                    session['wav_file_names'][
                        0]) + '/' + f'{diar_dir}/{meeting_id}_{sc_mc_tag}_{json_fname_suffix}.json'
        ...
        if all_gt_utt_df is not None:
            ...
            logger.debug(
                f"{session.session_id} tcp_WER: {session_wer['tcp_wer'].item()} tcorc_WER: {session_wer['tcorc_wer'].item()}")
