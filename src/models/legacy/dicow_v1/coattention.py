...
class Interaction(nn.Module):
    def __init__(self, config, num_layers=4):
        super().__init__()
        self.layers = nn.ModuleList([CoAttention(embed_dim=config.d_model, single_dim=config.d_model,
                                                 multi_dim=config.d_model // 2, n_heads=config.encoder_attention_heads)
                                     for _ in range(num_layers)])

    def forward(self, hidden_states, per_group_sizes):
        orig_shape = hidden_states.shape
        max_spk = per_group_sizes.max()
        hidden_states = hidden_states.reshape(-1, max_spk, *hidden_states.shape[1:]).permute(2, 0, 1, 3)
        # hidden_states = hidden_states.reshape(-1, per_group_sizes[0], *hidden_states.shape[1:]).permute(2, 0, 1, 3)
        for layer in self.layers:
            hidden_states = layer(hidden_states, per_group_sizes)
        hidden_states = hidden_states.permute(1, 2, 0, 3).reshape(orig_shape)
