...
class DiCoWEncoder(WhisperEncoder):
    ...
    def forward(
        ...
        lang_encoder_out = self.lang_encoder(
            input_features,
        ...
        # 1. Create a new tensor of shape [B, T, 4] filled with zeros.
        # This automatically handles the 'O' class (index 3), so we don't have to touch it.
        stno = torch.zeros((B, T, 4), dtype=sme.dtype, device=sme.device)

        # 2. Reshape the mask to [Batch, 1] so PyTorch broadcasts it across the Time dimension
        is_embed_mask = is_embed.view(-1, 1)

        # S: Always takes probabilities from dim 0
        stno[..., 0] = sme[..., 0]

        # T: Takes dim 1 if embedded, else takes dim 2
        stno[..., 1] = torch.where(is_embed_mask, sme[..., 1], sme[..., 2])

        # N: Takes dim 2 if embedded, else takes dim 1
        stno[..., 2] = torch.where(is_embed_mask, sme[..., 2], sme[..., 1])
