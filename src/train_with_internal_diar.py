...
from models.containers_w_diar import WhisperContainer, get_optimizer
...
class StepInjectionCallback(TrainerCallback):
    def on_step_begin(self, args, state, control, model, **kwargs):
        # Handle distributed training wrappers (like DDP) safely
        base_model = model.module if hasattr(model, "module") else model

        # Inject the current step and max steps into the model
        base_model.current_training_step = state.global_step
        base_model.max_training_steps = state.max_steps
...
class ModelTrainer:
    ...
    def train(self):
        ...
        callbacks.append(StepInjectionCallback())
